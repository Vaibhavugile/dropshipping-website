...
/**
 * Admin Products UI
 * - Top: category chips + filters
 * - Dense grid: 8 cols, 2 rows visible
 * - Add / Edit via modal
 * - Image upload: drag & drop + file picker using firebaseService.uploadFiles
 */
...
export default function Products() {
  ...
  function selectAllOnPage() {
    setSelectedIds((prev) => {
      const c = new Set(prev);
      pageItems.forEach((it) => c.add(it.id));
      return c;
    });
  }
  ...
  async function deleteProduct(id) {
    ...
    try {
      ...
      setAllProducts(
        await firebaseService.listProductsByCategory(selectedCat.id)
      );
    } catch (err) {
      console.error(err);
      alert("Delete failed");
    ...
    <div className="prod-fullpage">
      <div className="prod-header">
        ...
        <p className="prod-subtitle">
          Browse products by category, manage inventory, and upload images.
        </p>
      </div>
      ...
      <div className="prod-topbar">
        ...
        <div className="controls-row">
          ...
          <div className="right-controls">
            <button className="btn ghost" onClick={selectAllOnPage}>
              Select page
            </button>
          ...
          <>
            <div className="grid">
              {pageItems.map((p) => (
                <div
                  key={p.id}
                  className={`card ${selectedIds.has(p.id) ? "selected" : ""}`}
                >
                  ...
                  <div className="card-body">
                    ...
                    <div className="card-stock">
                      Stock: <strong>{Number(p.stock || 0)}</strong>
                    </div>
                    <div className="card-variants">
                      {p.variants &&
                        ...
                        Object.entries(p.variants)
                          .slice(0, 2)
                          .map(([t, vals]) => (
                            <span key={t} className="variant-pill">
                              {t}: {Object.keys(vals).length}
                            </span>
                          ))}
                    </div>
                  </div>
            ...
            <div className="pager">
              ...
              <div className="page-info">
                Page {page} / {pageCount} — {filtered.length} products
              </div>
              <button
                className="page-btn"
...
function ProductModal({ categoryId, product, onClose }) {
  ...
  async function handleFiles(fileList) {
    ...
    for (const f of files) {
      setUploadQueue((prev) =>
        prev.map((q) => (q.file === f ? { ...q, status: "uploading" } : q))
      );
      try {
        const results = await firebaseService.uploadFiles(
          ...
          (prog) => {
            ...
            setUploadQueue((prev) =>
              prev.map((q) =>
                q.file === f ? { ...q, progress: prog.percent } : q
              )
            );
          }
        );
        ...
        setUploadQueue((prev) =>
          prev.map((q) =>
            q.file === f
              ? { ...q, status: "done", path: r.path, url: r.url }
              : q
          )
        );
        setForm((prev) => ({
          ...prev,
          images: [...(prev.images || []), r.url],
        }));
      } catch (err) {
        console.error("upload failed", err);
        setUploadQueue((prev) =>
          prev.map((q) =>
            q.file === f ? { ...q, status: "error" } : q
          )
        );
        ...
        <div className="modal-body">
          <div className="modal-left">
            ...
            <div className="prod-variant-box">
                  ...
                  {Object.entries(opts).map(([name, qty]) => (
                    <div key={name} className="variant-row">
                      ...
                      <input
                        ...
                        value={qty}
                        onChange={(e) =>
                          updateVariantQty(type, name, e.target.value)
                        }
                      />
                      <button
                        className="btn small ghost"
                        type="button"
                        onClick={() => removeVariant(type, name)}
                      >
                        ✕
                      </button>
                    </div>
              ...
              <div className="variant-type-actions">
                <button
                  className="prod-small-btn"
                  type="button"
                  onClick={() => addVariantType("colors")}
                >
                  + Color Variant
                </button>
                <button
                ...
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const t = prompt("Variant type (e.g. material)");
                    if (t) addVariantType(t.trim().toLowerCase());
                  }}
                >
                  + Custom
                </button>
          ...
          <div className="modal-right">
            ...
            <div className="preview-card">
              ...
              <div className="preview-body">
                ...
                <div className="preview-stock">Stock: {form.stock}</div>
                <div className="preview-variants">
                  {Object.keys(form.variants || {}).length > 0 ? (
                    Object.entries(form.variants).map(([t, opts]) => (
                      <div key={t}>
                        {t}: {Object.keys(opts).length} options
                      </div>
                    ))
                  ) : (
            ...
            <div className="preview-tips">
              <div className="preview-tips-title">Tips</div>
              <ul>
                <li>Drag multiple images or click “Choose files” to upload.</li>
                <li>The first image becomes the thumbnail.</li>
                <li>Edits update the product document; deletions are permanent.</li>
              </ul>
            </div>
          </div>
        </div>
